...
class CurrentElectionStateHandlerRegistratrionV1(CurrentElectionStateHandler):
    ...
    def customActions(self):
        ...
        election: Election = Election(date=datetime.fromisoformat(self.getStartTime()), status=electionStatusIDfromDB)
...
class CurrentElectionStateHandlerSeedingV1(CurrentElectionStateHandler):
    def __init__(self, data: dict):
        super().__init__(CurrentElectionState.CURRENT_ELECTION_STATE_SEEDING_V1, data, EdenBotMode.ELECTION)  # should be marked as election because of next step
