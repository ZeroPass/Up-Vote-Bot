...
class CommunityGroup:
    ...
    def getMembersRankFromChain(self) -> dict:
        """Get participants from chain, remove rank=0 participants and return a list of community participants"""
        try:
            ...
            if isinstance(response, ResponseSuccessful):
                ...
                for key, value in response.data.items():
                    try:
                        if len(value) != 2:
                            LOG.error("There is not enough values in response")
                            continue

                        if "account"  not in value[1] or value[1]['account'] is None:
                            LOG.error("There is no account in values")
                            continue

                        if "election_rank" not in value[1] or value[1]['election_rank'] is None:
                            LOG.error("There is no electionRank in values")
                            continue
                        if int(value[1]['election_rank']) == 0:
                            LOG.info("Election rank is 0, so we skip this participant")
                            continue

                        account: str = value[1]['account']
                        electionRank: int = (value[1]['election_rank'])
                    except Exception as e:
                        LOG.exception("Exception thrown when called getMembersFromChain.inner for loop" + str(e))

                    ranks[account] = electionRank
                return ranks
            else:
                raise CommunityGroupException("There was an error when getting members from chain")
        except Exception as e:
            LOG.exception(str(e))
            return None
    ...
    def merge(self,
        ...
        try:
            ...
            merged: list[CommunityParticipant] = list()
    ...
    def fromCustomMembersToCommunityParticipants(self, customMembers: list[CustomMember],
        ...
        try:
            ...
            for customMember in customMembers:
                ...
                found: list[Participant] = [x for x in participantsDB if len(x.telegramID) > 0 and REMOVE_AT_SIGN_IF_EXISTS(x.telegramID.lower()) ==
    ...
    def setTagAndAdminRights(self, participantsGoal: list[CommunityParticipant],
        ...
        try:
            ...
            maxLevel: int = 0 #head chief delegate
            for participant in participantsGoal:
                if participant.sbt.round > maxLevel:
                    maxLevel = participant.sbt.round
            LOG.debug("Highest level of last election is: " + str(maxLevel))
            headChiefDelegateLvl: int = maxLevel
            chiefDelegateLvl: int = maxLevel - 1
            highestNonChiefDelegateLvl: int = maxLevel - 2

            LOG.info("Head chief delegate level: " + str(headChiefDelegateLvl)
                     + " chief delegate level: " + str(chiefDelegateLvl)
                     + " highest non chief delegate level: " + str(highestNonChiefDelegateLvl))
            ...
            for index, participant in enumerate(participantsGoal):
                #current CD
                if participant.sbt.round == headChiefDelegateLvl or \
                        participant.sbt.round == chiefDelegateLvl:
                    LOG.success("Current CD: " + str(participant.accountName))
                    participantsGoal[index].customMember = \
                ...
                #last round tags
                elif participant.sbt.round == highestNonChiefDelegateLvl:
